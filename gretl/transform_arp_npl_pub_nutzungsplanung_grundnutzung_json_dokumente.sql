...
(
  SELECT
    dok_t_id as t_id, 
    --to_json(row_to_json(t)::text) AS json_dokument 
    row_to_json(t)::text AS json_dokument -- Text-Repräsentation des JSON-Objektes. 
  FROM
  (
    SELECT
      'SO_Nutzungsplanung_Publikation_20190116.Nutzungsplanung.Dokument'::text AS "@type",
      d.t_id AS dok_t_id, 
      ''::text AS "Id",
      d.titel AS "Titel",
      d.offiziellertitel AS "OffiziellerTitel",
      d.abkuerzung AS "Abkuerzung",
      d.offiziellenr AS "OffizelleNr",
      d.kanton AS "Kanton",
      d.gemeinde AS "Gemeinde",
      d.publiziertab AS "publiziertAb",
      d.rechtsstatus AS "Rechtsstatus",
      ('https://geo.so.ch/docs/ch.so.arp.zonenplaene/Zonenplaene_pdf/'||"textimweb"||'')::text AS "TextImWeb",
      d.bemerkungen AS "Bemerkungen",
      d.rechtsvorschrift AS "Rechtsvorschrift"
    FROM
      arp_npl.rechtsvorschrften_dokument AS d
  ) AS t
)
...
--SELECT * FROM typ_grundnutzung_json_dokument
,
...
(
  ...
  (
    SELECT
      ...
      string_agg(json_dokument::text, ',') AS dokumente
    FROM
...
--SELECT * FROM typ_grundnutzung_json_dokument_agg
,
